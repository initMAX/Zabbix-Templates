<!-- *********************************************************************************************************************************** -->
<!-- *** HEADER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<div align="center">
    <a href="http://www.initmax.com"><img src="./.readme/logo/initMAX_banner.png" alt="initMAX Logo"></a>
    <h3>
        <span>
            Honesty, diligence and MAXimum knowledge of our products is our standard.
        </span>
    </h3>
    <h3>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>&nbsp;
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
    </h3>
    <h3>
        <a><img src="./.readme/logo/zabbix-premium-partner.png" alt="Zabbix premium partner" width="100"></a>&nbsp;&nbsp;&nbsp;
        <a><img src="./.readme/logo/zabbix-certified-trainer.png" alt="Zabbix certified trainer" width="100"></a>
    </h3>
</div>
<br>
<br>

---
---

<div align="center">
    <h1>
        Zabbix Fail2ban Template
    </h1>
    <h4>
        Auto‑discovery & monitoring of Fail2ban jails with built‑in dashboard
    </h4>
    <br>
</div>
<br>
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** BODY ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Description
This template monitors Fail2ban on any Linux host using **a single `fail2ban‑client` call** per jail.  
It auto‑discovers all jails, tracks current/total bans & failed logins, triggers alerts on spikes, and ships with a ready‑made dashboard.

## Requirements
- **Zabbix 7.0 or higher**
- Zabbix **Agent** running on the monitored host
- Fail2ban package (incl. `fail2ban-client`) installed
- UserParameter file `Fail2ban.conf` placed in the agent’s *.d* directory
- Sudo rule allowing the agent to run `fail2ban-client` without password

## Installation
### 1. Deploy the configuration file  
Download the latest [Fail2ban.conf](./7.0/Fail2ban.conf) and place it in the correct directory.

Zabbix Agent
```bash
sudo wget https://git.initmax.cz/initMAX-Public/zabbix-templates/-/raw/production/free/Fail2ban/7.0/Fail2ban.conf -O /etc/zabbix/zabbix_agentd.d/Fail2ban.conf
```
Zabbix Agent 2
```bash
sudo wget https://git.initmax.cz/initMAX-Public/zabbix-templates/-/raw/production/free/Fail2ban/7.0/Fail2ban.conf -O /etc/zabbix/zabbix_agent2.d/Fail2ban.conf
```

### 2. Grant access to Fail2ban  
Create a dedicated sudoers drop‑in file (safer than editing `/etc/sudoers` directly):

#### write the rules
```bash
cat <<'EOF' | sudo tee /etc/sudoers.d/zabbix-fail2ban >/dev/null
zabbix ALL=(root) NOPASSWD: /usr/bin/fail2ban-client status
zabbix ALL=(root) NOPASSWD: /usr/bin/fail2ban-client status *
EOF
```

#### set correct permissions
```bash
sudo chmod 440 /etc/sudoers.d/zabbix-fail2ban
```

#### Finally restart the agent to load the new user‑parameter:

```bash
sudo systemctl restart zabbix-agent      # or zabbix-agent2
```

### 3. Test the agent keys  
Run as *zabbix* user to verify discovery and status keys.

Zabbix Agent
```bash
sudo -u zabbix zabbix_agent  -t fail2ban.discovery
sudo -u zabbix zabbix_agent  -t fail2ban.status['sshd']
```
Zabbix Agent 2
```bash
sudo -u zabbix zabbix_agent2 -t fail2ban.discovery
sudo -u zabbix zabbix_agent2 -t fail2ban.status['sshd']
```

Example discovery output:  

```json
{"data":[{"{#JAIL}":"imapd"},{"{#JAIL}":"sshd"}]}
```

### 4. Import template 

If the tests return data, **import** [template](./7.0/Template_Fail2ban.yaml) and link it to the host—metrics, triggers and dashboard will appear automatically.

Assign it to your Linux host.

Optionally tune macro thresholds (see below).

### Macros used
| Macro | Description | Default |
|-------|-------------|---------|
| `{$FAIL2BAN.MATCHES}` | Regex to **include** jails in discovery | `.*` |
| `{$FAIL2BAN.NOT_MATCHES}` | Regex to **exclude** jails | `TEST\|TEST2` |
| `{$CURRENTLY.BANNED.IPS.WARN}` | **Warning** trigger when current banned IPs exceed this number | `5` |
| `{$CURRENTLY.BANNED.IPS.EXCEEDS}` | **High** trigger when current banned IPs exceed this number | `100` |
| `{$SPIKE.IN.FAILED.LOGINS}` | Trigger when new failed logins exceed N within the window | `10` |
| `{$SPIKE.IN.FAILED.LOGINS_TIME}` | Time window for spike trigger (e.g. 5m) | `5m` |

### Per-jail thresholds (user macro context)

The ban-count thresholds are evaluated per discovered jail, so a single global value rarely fits all of them. Bot/scanner jails (e.g. `nginx-botsearch`, `nginx-custom-badbots`) routinely keep a handful of IPs banned, which would otherwise create constant Warning noise.

Override the threshold for a specific jail by adding a **macro with context** on the host or template - the context is the `{#JAIL}` name:

| Macro | Effect |
|-------|--------|
| `{$CURRENTLY.BANNED.IPS.WARN:nginx-botsearch}` = `50` | Raise the Warning threshold for the noisy `nginx-botsearch` jail |
| `{$CURRENTLY.BANNED.IPS.WARN:dovecot}` = `20` | Tolerate more bans on `dovecot` before warning |
| `{$CURRENTLY.BANNED.IPS.EXCEEDS:nginx-custom-badbots}` = `500` | Raise the High threshold for `nginx-custom-badbots` |
| `{$CURRENTLY.BANNED.IPS.WARN}` (no context) | Default applied to every other jail |

Jails without a matching context macro fall back to the plain (context-less) default, so you only override the noisy ones.

<!-- *********************************************************************************************************************************** -->
<!-- *** FOOTER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<br>
<br>

---
---
<div align="center">
    <h4>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="tel:+420800244442">
            <img alt="Static Badge" src="https://img.shields.io/badge/+420%20800%20244%20442-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="mailto:info@initmax.com">
            <img alt="Static Badge" src="https://img.shields.io/badge/info@initmax.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <br>
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
        <br><br><br>
        <a>
            <img src="./.readme/logo/agplv3.png" width="100">
        </a>
    </h4>
</div>
