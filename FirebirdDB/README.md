<!-- *********************************************************************************************************************************** -->
<!-- *** HEADER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<div align="center">
    <a href="http://www.initmax.com"><img src="./.readme/logo/initMAX_banner.png" alt="initMAX Logo"></a>
    <h3>
        <span>
            Honesty, diligence and MAXimum knowledge of our products is our standard.
        </span>
    </h3>
    <h3>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>&nbsp;
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
    </h3>
    <h3>
        <a><img src="./.readme/logo/zabbix-premium-partner.png" alt="Zabbix premium partner" width="100"></a>&nbsp;&nbsp;&nbsp;
        <a><img src="./.readme/logo/zabbix-certified-trainer.png" alt="Zabbix certified trainer" width="100"></a>
    </h3>
</div>
<br>
<br>

---
---

<div align="center">
    <h1>
        Zabbix FirebirdDB Template
    </h1>
    <h4>
        Auto-discovery & deep monitoring of Firebird databases
    </h4>
    <br>
</div>
<br>
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** BODY ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Description
This template monitors Firebird databases (v3+) using **two lightweight Python helper scripts**:

* **`Firebird_Discovery.py`** – discovers every *.fdb* file (or aliases) and feeds them to Zabbix LLD.
* **`Firebird_Zabbix.py`** – collects more than 30 metrics per DB (I/O, memory, transactions, sweeps, encryption …).

All trigger thresholds are exposed as **user macros** so you can tune them per host. A pre-built dashboard is included next to graph prototypes.

## Requirements
- **Zabbix 7.0 or higher**
- Zabbix **Agent 2** (recommended) or Agent 1 on the DB host (or on a proxy that can reach the server)
- Firebird Server 3.x or newer (the scripts gracefully handle missing columns on older versions)
- Python 3 with the `firebirdsql` module (`pip install firebirdsql`)

## Installation
### 1 · Deploy the helper scripts
```bash
sudo wget https://git.initmax.cz/initMAX-Public/zabbix-templates/-/raw/production/free/FirebirdDB/7.0/Firebird_Discovery.py -O /etc/zabbix/zabbix_agent2.d/Firebird_Discovery.py
sudo wget https://git.initmax.cz/initMAX-Public/zabbix-templates/-/raw/production/free/FirebirdDB/7.0/Firebird_Zabbix.py   -O /etc/zabbix/zabbix_agent2.d/Firebird_Zabbix.py

# Make them executable (rwx for owner, rx for group)
sudo chmod 750 /etc/zabbix/zabbix_agent2.d/Firebird_*.py
# If you keep the default 750, make sure the files belong to root:zabbix
sudo chown root:zabbix /etc/zabbix/zabbix_agent2.d/Firebird_*.py
```
`750` = **rwx-r-x---** → *owner* can read/write/execute, *group* (zabbix) can read/execute, *others* – no access. If the files are not group‑owned by **zabbix**, use `755`.

### 2 · Add UserParameters
Create `/etc/zabbix/zabbix_agent2.d/FirebirdDB.conf` with:
```ini
# Metric collector
UserParameter=fbdb[*],/etc/zabbix/zabbix_agent2.d/Firebird_Zabbix.py   --host $1 --database $2 --user $3 --password $4 --check $5

# Low‑level discovery
UserParameter=firebird.discovery[*],/usr/bin/sudo /etc/zabbix/zabbix_agent2.d/Firebird_Discovery.py
```
Restart the agent:
```bash
sudo systemctl restart zabbix-agent2
```

### 3 · Grant sudo for `Firebird_Discovery.py`
```bash
cat <<'EOF' | sudo tee /etc/sudoers.d/zabbix-firebird >/dev/null
zabbix ALL=(ALL) NOPASSWD: /etc/zabbix/zabbix_agent2.d/Firebird_Discovery.py
EOF
sudo chmod 440 /etc/sudoers.d/zabbix-firebird
```

### 4 · Test keys
```bash
# Discovery
sudo -u zabbix zabbix_agent2 -t firebird.discovery
# One metric
sudo -u zabbix zabbix_agent2 -t fbdb[127.0.0.1,/var/lib/firebird/3.0/data/test.fdb,sysdba,masterkey,iostat_reads]
```
You should see valid JSON for discovery and a numeric value for the metric.

### 5 · Import the template & dashboard
Import [`Template_FirebirdDB.yaml`](./7.0/Template_FirebirdDB.yaml) (**contains graphs + dashboard**) and link it to your Firebird host. Metrics, triggers, graphs and the dashboard will appear automatically.

### Macros used
| Macro | Description | Default |
|-------|-------------|---------|
| `{$FIREBIRD.CONNSTRING}` | Host or DSN passed to the helper script | `127.0.0.1` |
| `{$FIREBIRD.DB.MATCHES}` | Regex **include** filter for DB names | `.+` |
| `{$FIREBIRD.DB.NOT_MATCHES}` | Regex **exclude** filter | `^\s$` |
| `{$FIREBIRD.LLD.PATH}` | Base directory scanned by discovery | `/var/lib/firebird/3.0/data` |
| `{$FIREBIRD.USER}` | DB user | `sysdba` |
| `{$FIREBIRD.PASSWORD}` | DB password | `masterkey` |
| `{$FB_TXN_GAP_MAX}` | Max allowed transaction gap | `500000` |
| `{$FB_SWEEP_ACTIVE_SEC}` | Seconds sweep may run before alert | `600` |
| `{$FB_FORCED_WRITES_DISABLED}` | Value that means forced-writes OFF | `0` |
| `{$FB_RESERVE_SPACE_DISABLED}` | Value that means reserve-space OFF | `0` |
| `{$FB_CACHE_HIT_MIN}` | Minimum cache hit ratio (%) | `90` |
| `{$FB_CRYPT_STATE_ALERT}` | `crypt_state` threshold (>1 = encrypt/decrypt) | `1` |
| `{$FB_REPLICA_MODE_ALERT}` | `replica_mode` value that means read-only | `2` |
| `{$FB_LOCK_CONFLICTS_MAX}` | Max allowed lock conflicts before alert | `0` |
| `{$FB_LONG_TXN_COUNT_MAX}` | Max allowed long-running transactions | `0` |

<!-- *********************************************************************************************************************************** -->
<!-- *** FOOTER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<br>
<br>

---
---
<div align="center">
    <h4>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="tel:+420800244442">
            <img alt="Static Badge" src="https://img.shields.io/badge/+420%20800%20244%20442-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="mailto:info@initmax.com">
            <img alt="Static Badge" src="https://img.shields.io/badge/info@initmax.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <br>
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
        <br><br><br>
        <a>
            <img src="./.readme/logo/agplv3.png" width="100">
        </a>
    </h4>
</div>
