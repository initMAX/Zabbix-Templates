<!-- *********************************************************************************************************************************** -->
<!-- *** HEADER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<div align="center">
    <a href="http://www.initmax.com"><img src="./.readme/logo/initMAX_banner.png" alt="initMAX Logo"></a>
    <h3>
        <span>
            Honesty, diligence and MAXimum knowledge of our products is our standard.
        </span>
    </h3>
    <h3>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>&nbsp;
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
    </h3>
    <h3>
        <a><img src="./.readme/logo/zabbix-premium-partner.png" alt="Zabbix premium partner" width="100"></a>&nbsp;&nbsp;&nbsp;
        <a><img src="./.readme/logo/zabbix-certified-trainer.png" alt="Zabbix certified trainer" width="100"></a>
    </h3>
</div>
<br>
<br>

---
---

<div align="center">
    <h1>
        Zabbix Synology Active Backup for Business Template
    </h1>
    <h4>
        Monitoring of Synology Active Backup for Business via SSH and SQLite
    </h4>
    <br>
</div>
<br>
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** BODY ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Description
This template monitors Synology Active Backup for Business (ABB) by querying the internal SQLite databases via SSH. It automatically discovers backup tasks and devices, tracks backup status, transferred data, duration, and alerts on failures or missed backups.

Only actively assigned devices are monitored -- removed or orphaned VMs are automatically excluded.

## Requirements
- **Zabbix 7.0+** with SSH agent support
- **Synology DSM 7.0+** with Active Backup for Business installed
- SSH access to Synology NAS with a dedicated monitoring user
- `sqlite3` binary available on the NAS (included by default in DSM)
- `sudo` configured for passwordless `sqlite3` access

## Installation

### 1. Create a dedicated monitoring user
In DSM Control Panel > User & Group, create a new user (e.g., `zabbix`):
- Add to the `administrators` group (required for SSH access on DSM)
- Deny access to all DSM applications (restricts web UI permissions)

<div align="center">
    <img src="./.readme/screen/DSM_user.png" alt="DSM User Creation" width="500">
</div>

### 2. Enable SSH
In DSM Control Panel > Terminal & SNMP > Terminal, enable SSH service.

### 3. Configure passwordless sudo for sqlite3
SSH into the NAS as an admin user and create a sudoers rule:
```bash
sudo tee /etc/sudoers.d/zabbix << 'EOF'
zabbix ALL=(ALL) NOPASSWD: /usr/bin/sqlite3
EOF
sudo chmod 440 /etc/sudoers.d/zabbix
```

**Important:** DSM updates may remove custom sudoers rules. To persist the configuration, create a triggered task in DSM Task Scheduler:
- **Event:** Boot-up
- **User:** root
- **Script:**
```bash
cat > /etc/sudoers.d/zabbix << 'EOF'
zabbix ALL=(ALL) NOPASSWD: /usr/bin/sqlite3
EOF
chmod 440 /etc/sudoers.d/zabbix
```
This ensures the sudo rule is re-applied automatically after every DSM update or reboot.

### 4. Import the Template
Download and import the [Synology ABB Template](./7.0/template_synology_abb_7.0.yaml) into your Zabbix server.

### 5. Link Template to Host
Assign the template to the Zabbix host representing your Synology NAS.

### 6. Configure Template Macros
Set the SSH credentials on the host:

| Macro | Description | Default |
|-------|-------------|---------|
| `{$ABB.SSH.USER}` | SSH username on Synology NAS | `zabbix` |
| `{$ABB.SSH.PASS}` | SSH password (set as secret macro on host) | *(empty)* |
| `{$ABB.VOLUME}` | Synology storage volume for Active Backup data | `volume1` |
| `{$ABB.INTERVAL}` | Data collection interval | `10m` |
| `{$ABB.NO.BACKUP.WARN}` | Time (seconds) after which missing backup triggers warning | `172800` (48h) |

## Monitoring scope

### Task discovery (LLD)
- Automatic discovery of all backup tasks
- Per-task: last status, success/warning/error count
- Per-task: start time, end time, duration

### Device discovery (LLD)
- Automatic discovery of all actively backed-up devices
- Per-device: last backup status, transferred bytes
- Per-device: start time, end time, duration
- Per-device: time since last backup

### Backup status codes

| Code | Status |
|------|--------|
| 1 | In progress |
| 2 | Successful |
| 3 | Cancelled |
| 4 | Failed |
| 5 | Warning |
| 8 | Partial success |

## Triggers

| Trigger | Severity |
|---------|----------|
| Backup FAILED on device | High |
| Backup WARNING on device | Warning |
| Backup CANCELLED on device | Warning |
| No backup for device in 48h | Warning |
| Task has errors | High |
| Pipeline reload failure (per task) | Warning |

<!-- *********************************************************************************************************************************** -->
<!-- *** FOOTER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<br>
<br>

---
---
<div align="center">
    <h4>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="tel:+420800244442">
            <img alt="Static Badge" src="https://img.shields.io/badge/+420%20800%20244%20442-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="mailto:info@initmax.com">
            <img alt="Static Badge" src="https://img.shields.io/badge/info@initmax.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <br>
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
        <br><br><br>
        <a>
            <img src="./.readme/logo/agplv3.png" width="100">
        </a>
    </h4>
</div>
