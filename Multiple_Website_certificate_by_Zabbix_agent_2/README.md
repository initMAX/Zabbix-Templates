...
<div align="center">
    ...
    <br>
    <img src="./.readme/screen/MultipleWebsiteCerts.png" width="1000"> <!-- !!! change !!! -->
</div>
<br>
<br>
...
<!-- *********************************************************************************************************************************** -->
## Description
This template monitors TLS/SSL certificates across multiple websites simultaneously. Based on the original Zabbix template "Website certificate by Zabbix agent 2", it provides two deployment options:

1. Item Discovery Mode (Default): Template automatically creates items to monitor all certificates on the host where the template is applied.
2. Host Discovery Mode: Optionally, you can activate discovery of virtual hosts, where a separate virtual host is created for each certificate.

Note: Switching between these modes is done in the template settings.

## Requirements
- Zabbix 6.0 or higher

## Setup
1. Download and import template (valid for the version)
   - Host Discovery Mode:
     - [7.0](./7.0/Multiple_Website_certificate_by_Zabbix_agent_2.yaml)
2. Assign on host

## Configuration
Set {$CERT.ARRAY} macro with your certificate targets using format:
(<hostname1>:<port1>:<ip1>),(<hostname2>:<port2>:<ip2>),(<hostname3>)

- Hostname: Required, FQDN of the website (e.g., example.com)
- Port: Optional, defaults to {$CERT.DEFAULT.PORT} (443) if omitted
- IP: Optional, allows checking certificates on specific IP addresses

## Macros used
|Name|Description|Default|
|----|-----------|-------|
|{$CERT.ARRAY}|Array of hostnames, IP addresses and ports to monitor for certificate validity.|`(<hostname1>:<port1>:<ip1>),(<hostname2>:<port2>:<ip2>)`|
|{$CERT.DEFAULT.PORT}|Default port if not specified in CERT.ARRAY entries.|`443`|
|{$CERT.EXPIRY.WARN}|Warning threshold: Number of days until the certificate expires.|`30`|
|{$CERT.EXPIRY.HIGH}|Critical threshold: Number of days until the certificate expires.|`7`|
