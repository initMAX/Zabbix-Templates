...
<div align="center">
    <h1>
        Zabbix Ubiquiti UniFi UAP Access Point Template
    </h1>
    <h4>
        SNMP monitoring of Ubiquiti UniFi access points
    </h4>
    <br>
</div>
<br>
...
<!-- *********************************************************************************************************************************** -->
## Description
SNMP v2c monitoring of Ubiquiti UniFi access points. Uses the UBNT-UniFi-MIB, FROGFOOT-RESOURCES-MIB
and standard IF-MIB OIDs (all bundled in the template - no MIB files need to be installed on the
Zabbix server).
...
- Wireless radios (LLD) - channel utilization and connected clients
- System information - name, uptime, contact, location

## Supported devices
U6-Pro, U6-Lite, U6-LR, U6-Mesh, U6-Enterprise, UAP-AC-Pro, UAP-AC-Lite, UAP-AC-LR, UAP-nanoHD,
UAP-FlexHD.

## Requirements
- **Zabbix 7.0 or higher**
- SNMP v2c enabled on the UniFi device
- Network access from the Zabbix server/proxy to the device on UDP/161

## Setup
1. Enable SNMP (v2c) on the UniFi controller/device and set a read-only community string.
2. In Zabbix, add an **SNMP interface** to the host and set the community (default macro
   `{$SNMP_COMMUNITY}`).
3. Import [`template_ubiquiti_unifi_uap_access_point_7.0.yaml`](./7.0/template_ubiquiti_unifi_uap_access_point_7.0.yaml) and link the template to the host.
4. Adjust the threshold macros if needed:

| Macro | Description | Default |
|---|---|---|
| `{$CPU.UTIL.CRIT}` | CPU utilization critical threshold (%) | `90` |
| `{$MEMORY.UTIL.MAX}` | Memory utilization threshold (%) | `90` |
| `{$CHANNEL.UTIL.WARN}` | Wireless channel utilization warning (%) | `80` |
| `{$CHANNEL.UTIL.CRIT}` | Wireless channel utilization critical (%) | `90` |
| `{$ICMP_LOSS_WARN}` | ICMP packet loss warning (%) | `20` |
| `{$IF.ERRORS.WARN}` | Interface errors warning threshold | `2` |
