...
<div align="center">
    ...
    <h4>
        Template evaluates Zabbix CVEs using the effective version (auto or macro override) <!-- !!! change !!! -->
    </h4>
    <br>
    <img src="./.readme/screen/CVE_threats.png" width="1000"> <!-- !!! change !!! -->
</div>
<br>
<br>
...
<!-- *********************************************************************************************************************************** -->
## Description
This template evaluates Zabbix CVEs for the effective version and generates detailed events (CVE, workarounds, fixed version, ZBX ticket, affected components). It uses LLD discovery to create CVE items and triggers automatically.  
By default it uses the actual server version from `zabbix["version"]`. If macro `{$ZBX.CVE.VERSION}` is set, it overrides the detected version.  
CVE data are loaded from `{$ZBX.CVE.URL}` (JSON endpoint, no HTML parsing), which makes it more stable.  
It also checks whether the Zabbix server is still supported and whether an update is available.

## Requirements
 - Zabbix 7.0 or higher

## Setup
1\. Download and import [template](./7.0/template_zabbix_CVE_threats.yaml)<!-- !!! change !!! -->

2\. Create host and assign this template  
    *Note:* For automatic version detection, assign it to the Zabbix Server host (it relies on `zabbix["version"]`). If you use `{$ZBX.CVE.VERSION}` (for example `7.0.22`), the template can be attached to any host.

3\. (Optional) Set macro `{$ZBX.CVE.VERSION}` if you want to override version detection (example: `7.0.22`)

### Macros used
|Name|Description|Default|
|----|-----------|-------|
|`{$ZBX.CVE.URL}`|CVE JSON URL|`https://assets.zabbix.com/files/zabbix_security_advisories.json`|
|`{$ZBX.CVE.VERSION}`|Override version (leave empty to use `zabbix[version]`). Example: `7.0.22`|` `|
