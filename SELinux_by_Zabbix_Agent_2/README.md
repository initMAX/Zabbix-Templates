...
<div align="center">
    <h1>
        SELinux by Zabbix Agent 2
    </h1>
    <h4>
        Basic monitoring of the SELinux package using Zabbix Agent 2
    </h4>
    <br>
</div>
<br>
...
<!-- *********************************************************************************************************************************** -->
## Description
This template allows basic monitoring of the SELinux package using Zabbix Agent 2. It checks the current SELinux mode (enforcing, permissive, disabled), the mode configured at boot, and monitors for AVC denial messages logged by auditd.

## Requirements
 - Zabbix 7.0 or higher
 - Zabbix Agent 2 installed on the monitored host - Need to be in Active MODE!!!
 - SELinux installed and enabled on the monitored host
 - Auditd service running and configured to log SELinux denials (often default)

## Setup
1\. Download and import [template](./7.0/template_SELinux_by_Zabbix_Agent_2.yaml)

2\. Assign the template to your Linux host monitored by Zabbix Agent 2.

3\. Ensure the Zabbix Agent has permissions to read `/var/log/selinux_denials.log` (or the configured log file for denials), `/sys/fs/selinux/enforce`, and `/etc/selinux/config`.
