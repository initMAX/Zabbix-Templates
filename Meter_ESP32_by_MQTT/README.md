...
<div align="center">
    <h1>
        Zabbix ESP32 Meter Template by MQTT
    </h1>
    <h4>
        Besic measurements ESP32 meter inspired by AI on the edge, project by JomJol.
    </h4>
    <br>
</div>
<br>
...
If error is anything other than "no error", the value is ignored by preprocessing so that only valid readings update trends.
## Description
This template integrates jomjol’s **AI-on-the-Edge** ESP32 meter with **Zabbix via MQTT** (Zabbix Agent 2 `mqtt.get`). It subscribes to a per-device workshop topic, extracts the current meter reading, computes **daily consumption**, and derives **instantaneous flow** (m³/s).

**Topic pattern**
`/summit/meter{$WORKSHOP.NUMBER}/main/json`

**Expected JSON payload**
```json
{ "raw": 123.456, "error": "no error" }
```

## Requirements
- **Zabbix 7.0 or higher**
- **Zabbix Agent 2 (active)** on the host that will subscribe to MQTT
- MQTT broker reachable at tcp://127.0.0.1:1883 (this is the default used in the template)
- If your broker runs elsewhere, edit the Meter MQTT RAW item key and replace 127.0.0.1 with your broker’s address.
- ESP32 device running AI‑on‑the‑Edge and publishing JSON to the topic shown above

## Installation
### 1. Import template 

**Import** [template](./7.0/Template_Meter_ESP32_by_MQTT.yaml)

**Link it to a host that runs Zabbix Agent 2 (active)**
   - Create or pick a host that can reach your MQTT broker.
   - Make sure the host uses **Zabbix agent (active)** checks.

Fill mandatory macro (see below).

### Macros used
| Macro | Description | Example |
|-------|-------------|---------|
| `{$WORKSHOP.NUMBER}` | Workshop device number appended to the MQTT topic | `192.168.1.123` |
