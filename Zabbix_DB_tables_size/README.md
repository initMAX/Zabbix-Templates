<!-- *********************************************************************************************************************************** -->
<!-- *** HEADER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<div align="center">
    <a href="http://www.initmax.com"><img src="./.readme/logo/initMAX_banner.png" alt="initMAX Logo"></a>
    <h3>
        <span>
            Honesty, diligence and MAXimum knowledge of our products is our standard.
        </span>
    </h3>
    <h3>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>&nbsp;
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
    </h3>
    <h3>
        <a><img src="./.readme/logo/zabbix-premium-partner.png" alt="Zabbix premium partner" width="100"></a>&nbsp;&nbsp;&nbsp;
        <a><img src="./.readme/logo/zabbix-certified-trainer.png" alt="Zabbix certified trainer" width="100"></a>
    </h3>
</div>
<br>
<br>

---
---
<div align="center">
    <h1> <!-- !!! change !!! -->
        Zabbix DB tables size
    </h1>
    <h4>
        Checks and monitors the sizes of Zabbix database tables based on SQL query executed by Zabbix Agent 2. <!-- !!! change !!! -->
    </h4>
    <br>
    <img src="./.readme/screen/dashboard.png" width="1000"> <!-- !!! change !!! -->
</div>
<br>
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** BODY ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Description
This template uses PostgreSQL Custom Query for monitoring Zabbix database tables size and growth. It discovers tables, differentiates between regular tables and TimescaleDB hypertables, and tracks their sizes. The template includes:

- **Automatic discovery of tables** controlled by the **{$TABLES.SIZE}** macro - only tables larger than this threshold (default: 10MB) will be discovered and monitored
- Support for both regular PostgreSQL tables and TimescaleDB hypertables
- **Optional compressed / uncompressed split** for hypertables controlled by the **{$TABLES.SPLIT.COMPRESSION}** macro - see the on-disk size of compressed chunks separately from uncompressed chunks
- **Chunk counts** for hypertables - total, compressed and uncompressed number of chunks
- **"Other" aggregate** - a discovered virtual `other` table summing the size of all tables at or below the **{$TABLES.SIZE}** threshold, so pie/honeycomb views add up to the full database size
- Dashboard with pie chart and graph visualizations of table sizes, including a stacked compressed vs uncompressed graph
- Dependent items for processing and filtering data
- Configurable PostgreSQL connection parameters

**SQL Query Path Configuration:**
```
Plugins.PostgreSQL.CustomQueriesEnabled=true
Plugins.PostgreSQL.CustomQueriesPath=/etc/zabbix/sql/
```

**Required SQL query file:** Place the `zbxdb_tables.sql` file in your SQL queries directory

**Key Macros:**
- **{$TABLES.SIZE}**: Defines the minimum size threshold (in MB) for table discovery. Only tables larger than this value will be included in monitoring. This allows you to focus on monitoring only the largest and most important tables, reducing unnecessary data collection.
- **{$TABLES.SPLIT.COMPRESSION}** (default: `false`): When set to `true`, each hypertable additionally gets two separate size items - `size (compressed)` and `size (uncompressed)` - so you can see the ratio between compressed and uncompressed chunks. The combined total `size` item is **always** kept as well, whether or not the split is enabled. Any other value collects only the combined size. Chunk-count items (total / compressed / uncompressed) are always collected for hypertables regardless of this macro. Regular (non-hypertable) tables are unaffected and always report a single combined size.

## Requirements
 - Zabbix 7.0 or higher

## Setup
1\. Download and import [template](./7.0/template_Zabbix_DB_tables_size.yaml)

2\. Create the SQL query directory if it doesn't exist (default: `/etc/zabbix/sql/`)

3\. Place the [`zbxdb_tables.sql`](./7.0/sql/zbxdb_tables.sql) file in the SQL queries directory

4\. Configure Zabbix Agent 2 with the correct PostgreSQL connection details in the host macros:
   - {$TABLES.PG.CONNSTRING}: Connection string (default: tcp://localhost:5432)
   - {$TABLES.PG.USER}: PostgreSQL username with read access to table metadata
   - {$TABLES.PG.PASSWORD}: Password for the PostgreSQL user
   - {$TABLES.PG.DATABASE}: Database name (default: zabbix)
   - **{$TABLES.SIZE}**: Minimum size threshold in MB for table discovery (default: 10) - **only tables larger than this value will be discovered and monitored**
   - **{$TABLES.SPLIT.COMPRESSION}**: Set to `true` to additionally collect separate compressed / uncompressed chunk size items for hypertables; the combined total size is always kept (default: `false`)

5\. Assign the template to your Zabbix server or proxy host


<!-- *********************************************************************************************************************************** -->
<!-- *** FOOTER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<br>
<br>

---
---
<div align="center">
    <h4>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="tel:+420800244442">
            <img alt="Static Badge" src="https://img.shields.io/badge/+420%20800%20244%20442-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="mailto:info@initmax.com">
            <img alt="Static Badge" src="https://img.shields.io/badge/info@initmax.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <br>
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
        <br><br><br>
        <a>
            <img src="./.readme/logo/agplv3.png" width="100">
        </a>
    </h4>
</div>
