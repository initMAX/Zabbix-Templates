<!-- *********************************************************************************************************************************** -->
<!-- *** HEADER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<div align="center">
    <a href="http://www.initmax.com"><img src="./.readme/logo/initMAX_banner.png" alt="initMAX Logo"></a>
    <h3>
        <span>
            Honesty, diligence and MAXimum knowledge of our products is our standard.
        </span>
    </h3>
    <h3>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>&nbsp;
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
    </h3>
    <h3>
        <a><img src="./.readme/logo/zabbix-premium-partner.png" alt="Zabbix premium partner" width="100"></a>&nbsp;&nbsp;&nbsp;
        <a><img src="./.readme/logo/zabbix-certified-trainer.png" alt="Zabbix certified trainer" width="100"></a>
    </h3>
</div>
<br>
<br>

---
---

<div align="center">
    <h1>
        Zabbix Media HTML Template
    </h1>
    <h4>
        Clean, structured HTML notifications for Problem, Recovery and Update
    </h4>
    <br>
    <img src="./.readme/screen/problem.png" width="32%" alt="Problem notification">
    <img src="./.readme/screen/update.png" width="32%" alt="Update notification">
    <img src="./.readme/screen/recovery.png" width="32%" alt="Recovery notification">
</div>
<br>
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** BODY ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Description
This media type provides a reusable HTML notification layout for Zabbix. The message bodies are separated
into individual files (Problem, Recovery, Update) for easy editing and versioning.

**Package version:** 7.0-1

## Key features
- **Severity-aware colors** on headers and bars to make priority instantly visible.
- **Outlook-safe table layout** with consistent spacing and full-width action buttons.
- **Light/dark mode aware** layout with automatic logo swap and contrast-safe colors.
- **Smart action buttons** with direct links to event, problems view, host inventory, and host graphs.
- **Latest data detection**: shows graph vs. values based on item value type and hides when trigger uses multiple items.
- **Optional fields** (e.g., trigger description) render only when available to keep emails clean.
- **Multi-item triggers** are detected to avoid misleading single-item shortcuts.

## Requirements
- **Zabbix 7.0.4 or higher** (required macro functions)

## Setup
1. If not already set, define global macro `{$ZABBIX.URL}` with your Zabbix frontend URL.  
   The value is used as the base URL for links to the Zabbix frontend and is set in **Administration → General → Macros**.
2. Import the media type from [`initmax_custom_html_email_mediatype.yaml`](./7.0/initmax_custom_html_email_mediatype.yaml).
3. Configure the SMTP server and sender settings for your environment.
4. Ensure your action uses **HTML** message format and test notifications.

## Structure
- `7.0/initmax_custom_html_email_mediatype.yaml` - Zabbix media type export
- `html/problem.html` - HTML for Problem notifications
- `html/recovery.html` - HTML for Recovery notifications
- `html/update.html` - HTML for Update notifications

Raw HTML content is stored in the `html/` directory as a reference only (templates already include it).

## Used macros
| Macro | Type | Purpose |
|-------|------|---------|
| `{$ZABBIX.URL}` | Global | Base URL for links to the Zabbix frontend (example: https://www.initmax.com/zabbix) |
| `{EVENT.NAME}` | Built-in | Problem name (Problem/Update) |
| `{EVENT.TIME}` | Built-in | Problem time (Problem) |
| `{EVENT.DATE}` | Built-in | Problem date (Problem) |
| `{EVENT.SEVERITY}` | Built-in | Severity name (Problem) |
| `{EVENT.ID}` | Built-in | Event ID (Problem/Update) |
| `{EVENT.OPDATA}` | Built-in | Operational data (Problem) |
| `{EVENT.TAGS}` | Built-in | Event tags (Problem/Update) |
| `{EVENT.STATUS}` | Built-in | Problem status (Update) |
| `{EVENT.ACK.STATUS}` | Built-in | Acknowledged status (Update) |
| `{EVENT.UPDATE.TIME}` | Built-in | Update time (Update) |
| `{EVENT.UPDATE.DATE}` | Built-in | Update date (Update) |
| `{EVENT.UPDATE.ACTION}` | Built-in | Update action (Update) |
| `{EVENT.UPDATE.MESSAGE}` | Built-in | Update message (Update) |
| `{EVENT.RECOVERY.NAME}` | Built-in | Recovery event name (Recovery) |
| `{EVENT.RECOVERY.TIME}` | Built-in | Recovery time (Recovery) |
| `{EVENT.RECOVERY.DATE}` | Built-in | Recovery date (Recovery) |
| `{EVENT.RECOVERY.ID}` | Built-in | Recovery event ID (Recovery) |
| `{EVENT.RECOVERY.TAGS}` | Built-in | Recovery tags (Recovery) |
| `{EVENT.RECOVERY.STATUS}` | Built-in | Recovery status text (Recovery) |
| `{EVENT.DURATION}` | Built-in | Problem duration (Recovery) |
| `{TRIGGER.ID}` | Built-in | Trigger ID (links) |
| `{TRIGGER.DESCRIPTION}` | Built-in | Trigger description |
| `{TRIGGER.SEVERITY}` | Built-in | Trigger severity (Update bar) |
| `{HOST.ID}` | Built-in | Host ID (links) |
| `{HOST.NAME}` | Built-in | Host name |
| `{HOST.CONN}` | Built-in | Host connection info |
| `{USER.FULLNAME}` | Built-in | User full name (Update) |
| `{ITEM.ID1}` | Built-in | First item ID from trigger expression |
| `{ITEM.ID2}` | Built-in | Second item ID (used to hide single-item link) |
| `{ITEM.VALUETYPE}` | Built-in | Item value type (selects graph vs. values) |

Macro functions used in templates: `htmlencode()`, `urlencode()`, `regsub()`.

<!-- *********************************************************************************************************************************** -->
<!-- *** FOOTER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<br>
<br>

---
---
<div align="center">
    <h4>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="tel:+420800244442">
            <img alt="Static Badge" src="https://img.shields.io/badge/+420%20800%20244%20442-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="mailto:info@initmax.com">
            <img alt="Static Badge" src="https://img.shields.io/badge/info@initmax.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <br>
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
        <br><br><br>
        <a>
            <img src="./.readme/logo/agplv3.png" width="100">
        </a>
    </h4>
</div>
