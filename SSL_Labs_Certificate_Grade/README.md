...
<!-- *********************************************************************************************************************************** -->
## Description
This template performs external TLS/SSL health checks against any public‑facing FQDN using the Qualys SSL Labs API.
It retrieves the current certificate grade (A+ → F), verifies basic HTTPS reachability, and exposes a direct link to the full SSL Labs report from every Zabbix problem event.

## Requirements
 - Zabbix 7.0 or newer (relies on Script item type).
 - Zabbix Server or Proxy must have Internet access to api.ssllabs.com and www.ssllabs.com.
 - A corporate e‑mail address for API registration (the template auto‑registers it the first time).
 - Target service must listen on 443 (default) or 8443 / 10443—other ports are not supported by the public SSL Labs API.

## Setup
1\. Download and import [template](./7.0/Template_SSL_Labs_–_Certificate_Grade.yaml)

2\. Assign the template to any host that represents the domain you want to test.

3\. Define three macros on the host or template level:

### Macros used

| Macro              | Description                                                                                     | Default              |
|--------------------|-------------------------------------------------------------------------------------------------|----------------------|
| `{$GRADE.DOMAIN}`  | FQDN to be scanned, e.g.&nbsp;`www.example.com`                                                  | `<FQDN_TO_TEST>`     |
| `{$GRADE.EMAIL}`   | Corporate e‑mail address passed to the SSL Labs API (auto‑registered on first run)               | `<EMAIL_FOR_SSL_LABS>` |
| `{$GRADE.PORT}`    | TCP port – leave empty or set to `443` for default HTTPS.<br>Only **8443** and **10443** are accepted as non‑standard alternatives by the public API. | `443` |
