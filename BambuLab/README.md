<!-- *********************************************************************************************************************************** -->
<!-- *** HEADER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<div align="center">
    <a href="http://www.initmax.com"><img src="./.readme/logo/initMAX_banner.png" alt="initMAX Logo"></a>
    <h3>
        <span>
            Honesty, diligence and MAXimum knowledge of our products is our standard.
        </span>
    </h3>
    <h3>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>&nbsp;
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
    </h3>
    <h3>
        <a><img src="./.readme/logo/zabbix-premium-partner.png" alt="Zabbix premium partner" width="100"></a>&nbsp;&nbsp;&nbsp;
        <a><img src="./.readme/logo/zabbix-certified-trainer.png" alt="Zabbix certified trainer" width="100"></a>
    </h3>
</div>
<br>
<br>

---
---

<div align="center">
    <h1>
        Zabbix BambuLab Template
    </h1>
    <h4>
        Passive monitoring of BambuLab printers via Zabbix Agent 2 MQTT plugin
    </h4>
    <br>
</div>
<br>
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** BODY ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Description
This template monitors BambuLab 3D printers via MQTT using a Zabbix Agent 2 plugin.  
It collects print progress, temperatures, fans, AMS trays, lights, WiFi status, and ships with a ready-made dashboard.

Note: It works only as a passive check (Zabbix server or proxy polls the agent).

## Requirements
- **Zabbix 7.0 or higher**
- Zabbix **Agent 2** running on a host that can reach the printer
- Printer IP address, serial number (SN), and MQTT access code
- Correct Go plugin binary for your platform (**amd64** or **arm64**)

## Access details
- **Access code (password):** Bambu Handy app -> Devices -> select printer -> Settings -> Access code
- **Serial number (SN):** printer label or Bambu Handy app -> Devices -> select printer -> Printer info
- **MQTT username:** default is `bblp` and usually does not need to change

## Installation

### 1. Check CPU architecture
```bash
uname -m
```
`x86_64` = amd64, `aarch64` = arm64.

### 2. Download the plugin binary (pick only one)
Download only one Go plugin binary for your architecture. Do not download both.
If `/usr/libexec/zabbix` does not exist (no other Agent 2 plugins installed), create it first.

```bash
sudo mkdir -p /usr/libexec/zabbix
```

**amd64 (x86_64)**
```bash
sudo wget https://git.initmax.cz/initMAX-Public/zabbix-templates/-/raw/production/free/BambuLab/7.0/dist/zabbix_agent2_plugin_bambulab_linux_amd64 -O /usr/libexec/zabbix/zabbix_agent2_plugin_bambulab_linux_amd64
```
```bash
sudo chown root:root /usr/libexec/zabbix/zabbix_agent2_plugin_bambulab_linux_amd64
```
```bash
sudo chmod 755 /usr/libexec/zabbix/zabbix_agent2_plugin_bambulab_linux_amd64
```
**arm64 (aarch64)**
```bash
sudo wget https://git.initmax.cz/initMAX-Public/zabbix-templates/-/raw/production/free/BambuLab/7.0/dist/zabbix_agent2_plugin_bambulab_linux_arm64 -O /usr/libexec/zabbix/zabbix_agent2_plugin_bambulab_linux_arm64
```
```bash
sudo chown root:root /usr/libexec/zabbix/zabbix_agent2_plugin_bambulab_linux_arm64
```
```bash
sudo chmod 755 /usr/libexec/zabbix/zabbix_agent2_plugin_bambulab_linux_arm64
```

### 3. Download and edit the plugin config
```bash
sudo wget https://git.initmax.cz/initMAX-Public/zabbix-templates/-/raw/production/free/BambuLab/7.0/bambulab.conf -O /etc/zabbix/zabbix_agent2.d/plugins.d/bambulab.conf
sudo chown zabbix:zabbix /etc/zabbix/zabbix_agent2.d/plugins.d/bambulab.conf
```
Open `bambulab.conf` and **uncomment only one line** that matches your architecture (amd64 or arm64).  
Only one plugin line should be enabled.

Example edit:
```bash
sudo nano /etc/zabbix/zabbix_agent2.d/plugins.d/bambulab.conf
```
Uncomment the amd64 line if your CPU is `x86_64`:
```text
Plugins.BambuLab.Path=/usr/libexec/zabbix/zabbix_agent2_plugin_bambulab_linux_amd64
```

### 4. Test agent config (optional)
```bash
sudo -u zabbix zabbix_agent2 -T
```

### 5. Restart the agent
```bash
sudo systemctl restart zabbix-agent2
```

### 6. Test the plugin key (optional)
Replace the IP, serial number, and access code with your values.
```bash
sudo -u zabbix zabbix_agent2 -t bambulab.get["tls://192.168.1.50:8883","device/0123456789/report","bblp","ACCESS_CODE"]
```

### 7. Import template
If the test returns data, import [template](./7.0/template_bambulab_agent2_7.0.yaml) and link it to the host.

Optionally tune macro thresholds (see below).

### Macros used
| Macro | Description | Default |
|-------|-------------|---------|
| `{$BAMBU_INTERVAL}` | Master item polling interval | `1m` |
| `{$BAMBU_PASS}` | Bambu access code / MQTT password | `-` |
| `{$BAMBU_SN}` | Printer serial number used to build MQTT topic `device/<SN>/report` | `0948BB531701177` |
| `{$BAMBU_TIMEOUT}` | Agent item timeout (must be <= agent Timeout) | `15s` |
| `{$BAMBU_URL}` | MQTT broker URL, e.g. `tls://192.168.220.75:8883` | `tls://192.168.220.75:8883` |
| `{$BAMBU_USER}` | MQTT username (Bambu LAN mode usually uses `bblp`) | `bblp` |
| `{$BAMBU_NODATA}` | No data trigger period for MQTT status | `5m` |
| `{$BAMBU_WIFI_WARN}` | WiFi RSSI warning threshold in dBm | `-75` |
| `{$BAMBU_WIFI_WARN_FOR}` | WiFi RSSI warning duration | `30m` |
| `{$BAMBU_WIFI_CRIT}` | WiFi RSSI critical threshold in dBm | `-80` |
| `{$BAMBU_WIFI_CRIT_FOR}` | WiFi RSSI critical duration | `10m` |
| `{$BAMBU_NOZZLE_TARGET_MIN}` | Minimum nozzle target temperature to check heating issues (C) | `150` |
| `{$BAMBU_NOZZLE_DELTA}` | Nozzle temperature delta to trigger heating warning (C) | `15` |
| `{$BAMBU_NOZZLE_HEAT_TIME}` | Nozzle heating timeout window | `5m` |
| `{$BAMBU_BED_TARGET_MIN}` | Minimum bed target temperature to check heating issues (C) | `40` |
| `{$BAMBU_BED_DELTA}` | Bed temperature delta to trigger heating warning (C) | `10` |
| `{$BAMBU_BED_HEAT_TIME}` | Bed heating timeout window | `10m` |
| `{$BAMBU_PAUSE_TIME}` | Print paused duration before raising a warning | `10m` |
| `{$BAMBU_DOOR_OPEN_TIME}` | Door open duration while printing before raising a warning | `2m` |

Metrics, triggers, discovery, and the dashboard will populate automatically.

<!-- *********************************************************************************************************************************** -->
<!-- *** FOOTER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<br>
<br>

---
---
<div align="center">
    <h4>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="tel:+420800244442">
            <img alt="Static Badge" src="https://img.shields.io/badge/+420%20800%20244%20442-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="mailto:info@initmax.com">
            <img alt="Static Badge" src="https://img.shields.io/badge/info@initmax.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <br>
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
        <br><br><br>
        <a>
            <img src="./.readme/logo/agplv3.png" width="100">
        </a>
    </h4>
</div>
