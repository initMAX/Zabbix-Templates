<!-- *********************************************************************************************************************************** -->
<!-- *** HEADER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<div align="center">
    <a href="http://www.initmax.com"><img src="./.readme/logo/initMAX_banner.png" alt="initMAX Logo"></a>
    <h3>
        <span>
            Honesty, diligence and MAXimum knowledge of our products is our standard.
        </span>
    </h3>
    <h3>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>&nbsp;
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
    </h3>
    <h3>
        <a><img src="./.readme/logo/zabbix-premium-partner.png" alt="Zabbix premium partner" width="100"></a>&nbsp;&nbsp;&nbsp;
        <a><img src="./.readme/logo/zabbix-certified-trainer.png" alt="Zabbix certified trainer" width="100"></a>
    </h3>
</div>
<br>
<br>

---
---

<div align="center">
    <h1>
        Zabbix EHEIM Digital Template
    </h1>
    <h4>
        Auto-discovery & monitoring of EHEIM Digital aquarium devices
    </h4>
    <br>
</div>
<br>
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** BODY ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Description
This template monitors various EHEIM Digital aquarium devices (chillers, feeders, LED controllers, filters, water quality sensors) via their HTTP API, providing automatic device discovery, operational status, and alerting.

## Features
- **Automatic device discovery** via EHEIM Digital API
- **Operational Mode Detection:** heating, cooling, or idle modes
- **Detailed Metrics:** temperature, feeder levels, pH status, UV service intervals, water conductance & pollution
- **Alert Triggers:** temperature thresholds, feeder levels, device communication issues

## Requirements
- **Zabbix 7.0+** with HTTP agent support
- Connectivity to EHEIM Digital API

## Installation

### 1. Import the Template
Download and import the [EHEIM Digital Template](./7.0/EHEIM_Digital_template.yaml) into your Zabbix server.

### 2. Configure Template Macros
Adjust template macros to match your environment:

| Macro | Description | Default |
|-------|-------------|---------|
| `{$API.HOST}` | API endpoint URL/IP | `192.168.1.1` |
| `{$API.USER}` | API username | `api` |
| `{$API.PASS}` | API password | `admin` |
| `{$API.NO.DATA}` | No data trigger interval | `15m` |
| `{$API.NO.DATA.MSTR}` | No data interval for master item (shorter than LLD) | `5m` |
| `{$AQUAALERT.MAX.POLLUTION}` | Max. water pollution level (%) for alerts | `90` |
| `{$AQUAALERT.SERVICE.SENSOR}` | Min. days until sensor service for alerts | `5` |
| `{$AQUAALERT.SERVICE.WATERCHANGE}` | Min. days until water change for alerts | `5` |
| `{$FILTER.MEASUREMENT}` | Filter measurement unit (litres or gallons) | `litres` |
| `{$FILTER.MODEL}` | Filter model ("700", "600T", "450", "350") | `600T` |
| `{$FILTER.SERVICE.TIME}` | Remaining service days for filter alert | `30` |
| `{$MAX.PH}` | Upper pH limit for alerts | `7` |
| `{$MIN.PH}` | Lower pH limit for alerts | `6.2` |
| `{$MAX.TEMP}` | Upper temperature limit for alerts (°C) | `30` |
| `{$MIN.TEMP}` | Lower temperature limit for alerts (°C) | `20` |
| `{$MIN.FEEDER.FILL}` | Minimum feeder fill percentage for alerts | `20` |
| `{$PH.SERVICE.TIME}` | Remaining pH-probe service days | `5` |
| `{$TEMP.AVG.TIME}` | Averaging window for temperature checks | `15m` |
| `{$TRIGGER.PH.TIME}` | Averaging window for pH checks | `4h` |
| `{$UV.SERVICE.TIME}` | Remaining UV-unit service days for alerts | `30` |

### 3. Link Template to Host
Assign the imported template to your Zabbix host.

Metrics, triggers, and device discovery will automatically populate.

<!-- *********************************************************************************************************************************** -->
<!-- *** FOOTER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<br>
<br>

---
---
<div align="center">
    <h4>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="tel:+420800244442">
            <img alt="Static Badge" src="https://img.shields.io/badge/+420%20800%20244%20442-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="mailto:info@initmax.com">
            <img alt="Static Badge" src="https://img.shields.io/badge/info@initmax.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <br>
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
        <br><br><br>
        <a>
            <img src="./.readme/logo/agplv3.png" width="100">
        </a>
    </h4>
</div>
