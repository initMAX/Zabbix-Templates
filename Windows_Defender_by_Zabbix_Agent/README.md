<!-- *********************************************************************************************************************************** -->
<!-- *** HEADER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<div align="center">
    <a href="http://www.initmax.com"><img src="./.readme/logo/initMAX_banner.png" alt="initMAX Logo"></a>
    <h3>
        <span>
            Honesty, diligence and MAXimum knowledge of our products is our standard.
        </span>
    </h3>
    <h3>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>&nbsp;
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
    </h3>
    <h3>
        <a><img src="./.readme/logo/zabbix-premium-partner.png" alt="Zabbix premium partner" width="100"></a>&nbsp;&nbsp;&nbsp;
        <a><img src="./.readme/logo/zabbix-certified-trainer.png" alt="Zabbix certified trainer" width="100"></a>
    </h3>
</div>
<br>
<br>

---
---

<div align="center">
    <h1>
        Zabbix Windows Defender Template
    </h1>
    <h4>
        Monitoring Microsoft Windows Defender via Zabbix Agent
    </h4>
    <br>
</div>
<br>
<br>

<!-- *********************************************************************************************************************************** -->
<!-- *** BODY ************************************************************************************************************************** -->
<!-- *********************************************************************************************************************************** -->
## Description
This template monitors Microsoft Windows Defender on Windows hosts via Zabbix Agent (active and
passive checks). Protection status and signature/scan health are derived from the
`MSFT_MpComputerStatus` WMI/PowerShell data, and Defender security events are read from the Windows
Event Log. The event part is based on the Zabbix
[community template](https://github.com/zabbix/community-templates/tree/main/Operating_Systems/Windows/template_windows_defender_and_antimalware_eventlog_monitoring/6.0).

## What is monitored
- **Protection states** - Real-Time, Anti-Virus, Anti-Spyware, Behavior Monitor, IOAV, NIS and
  On-Access protection (enabled/disabled).
- **Signatures** - AntiVirus / AntiSpyware / NIS signature age and last-updated time.
- **Scans** - last quick scan and full scan age; scan started / completed / cancelled / failed.
- **Threats & events** - malware detected, action taken / failed / critically failed, suspicious
  behavior and history cleared (from the Defender event log).
- **Health** - overall Defender healthy state, engine and real-time-protection failures, signature
  update failures, platform out of date, antimalware and Defender service state.

## Requirements
- **Zabbix 7.0 or higher**
- Windows host with **Zabbix Agent or Agent 2** (active checks recommended for the event-log items)
- Microsoft Windows Defender / antimalware enabled on the host

## Setup
1. Import [`template_windows_defender_7.0.yaml`](./7.0/template_windows_defender_7.0.yaml) and link
   it to the Windows host monitored by Zabbix Agent (with active checks enabled).
2. Adjust the host macros if needed:

| Macro | Description | Default |
|---|---|---|
| `{$AV_EVENTLOG}` | Windows Defender event log channel | `Microsoft-Windows-Windows Defender/Operational` |
| `{$AV_SOURCE}` | Event source filter (`None` = all sources) | `None` |
| `{$FULL.SCAN.AGE.LIMIT}` | Max full scan age before an alert is raised (days) | `5` |
| `{$QUICK.SCAN.AGE.LIMIT}` | Max quick scan age before an alert is raised (days) | `3` |
| `{$SIGNATURE.AGE.LIMIT}` | Max signature age before a warning is raised (days) | `90` |

3. Make sure the agent can read the Defender status (`MSFT_MpComputerStatus`) and the Windows event
   log. After a couple of polling intervals the data appears in **Monitoring -> Latest data**.

<!-- *********************************************************************************************************************************** -->
<!-- *** FOOTER ************************************************************************************************************************ -->
<!-- *********************************************************************************************************************************** -->
<br>
<br>

---
---
<div align="center">
    <h4>
        <a href="https://www.initmax.com/">
            <img alt="Static Badge" src="https://img.shields.io/badge/initMAX.com-%20?color=%231f65f4&logo=data:image/png;base64,">
        </a>
        <a href="tel:+420800244442">
            <img alt="Static Badge" src="https://img.shields.io/badge/+420%20800%20244%20442-%20?color=%231f65f4&logo=data:image/svg+xml;base64,">
        </a>
        <a href="mailto:info@initmax.com">
            <img alt="Static Badge" src="https://img.shields.io/badge/info@initmax.com-%20?color=%231f65f4&logo=data:image/svg+xml;base64,">
        </a>
        <br>
        <a href="https://www.linkedin.com/company/initmax/">
            <img alt="LinkedIn" src="https://img.shields.io/badge/%20-%20?style=social&logo=data:image/svg+xml;base64,">
        </a>&nbsp;
        <a href="https://www.youtube.com/@initmax1">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-web?style=social&logo=youtube">
        </a>&nbsp;
        <a href="https://www.facebook.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=facebook">
        </a>&nbsp;
        <a href="https://www.instagram.com/initmax/">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=instagram">
        </a>&nbsp;
        <a href="https://twitter.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=x">
        </a>&nbsp;
        <a href="https://github.com/initmax">
            <img alt="Static Badge" src="https://img.shields.io/badge/%20-%20?style=social&logo=github">
        </a>
        <br><br><br>
        <a>
            <img src="./.readme/logo/agplv3.png" width="100">
        </a>
    </h4>
</div>
