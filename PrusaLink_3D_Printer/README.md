...
<div align="center">
    <h1>
        Zabbix PrusaLink 3D Printer Template
    </h1>
    <h4>
        Monitoring Prusa 3D printers via PrusaLink HTTP API
    </h4>
    <br>
</div>
<br>
...
<!-- *********************************************************************************************************************************** -->
## Description
This template monitors Prusa 3D printers via PrusaLink HTTP API.
It uses `/api/v1/info`, `/api/v1/status`, `/api/v1/job`, and `/api/version` as master items
with dependent items for common metrics.
You must set the printer IP and credentials for it to work.

## Requirements
- Zabbix 7.0 or higher
- Prusa printer with PrusaLink enabled
- PrusaLink username/password (Digest auth)
- Printer IP address

## Setup
1. Enable PrusaLink and note the username/password
2. Import the template
3. Set macros and link the template to the host

## Template
- [7.0](./7.0/template_prusalink_3d_printer_7.0.yaml)

### Macros used
| Macro | Description | Default | Example |
|-------|-------------|---------|---------|
| `{$PRUSA.HTTP.SCHEMA}` | HTTP schema (http/https) | `http` | `https` |
| `{$PRUSA.IP}` | PrusaLink IP address (required) | (empty) | `192.168.1.50` |
| `{$PRUSA.USERNAME}` | PrusaLink username (required) | `maker` | `maker` |
| `{$PRUSA.PASSWORD}` | PrusaLink password (required) | (empty) | `your-password` |
| `{$PRUSA.NOZZLE.TEMP.WARN}` | Nozzle temperature warning threshold (C) | `260` | `250` |
| `{$PRUSA.NOZZLE.TEMP.HIGH}` | Nozzle temperature high threshold (C) | `280` | `270` |
| `{$PRUSA.BED.TEMP.WARN}` | Bed temperature warning threshold (C) | `100` | `80` |
| `{$PRUSA.BED.TEMP.HIGH}` | Bed temperature high threshold (C) | `110` | `100` |
