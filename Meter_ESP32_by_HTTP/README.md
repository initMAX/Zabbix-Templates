...
<div align="center">
    <h1>
        Zabbix ESP32 Meter Template by HTTP
    </h1>
    <h4>
...
It collects raw JSON from the ESP32 device, extracts current meter status, calculates daily consumption, and tracks water stream (m³/s).

## Requirements
- **Zabbix 7.0 or higher**
- ESP32 device running jomjol AI-on-the-Edge firmware
- Device reachable over HTTP ({$ESP32.IP} macro)
- JSON endpoint /json must be enabled

## Installation
### 1. Import template 

**Import** [template](./7.0/Template_Meter_ESP32_by_HTTP.yaml) and link it to your host.
...
|-------|-------------|---------|
| `{$ESP32.IP}` | ESP32 device IP address | `192.168.1.123` |
